...
const effectiveFromDate = computed(() => fromDate.value || maxDate.value);
...
const history = computed(() =>
    reportsStore.buildOperationalHistory(activeOrganizationId.value, {
...
const hasEvents = computed(() => history.value.totalEvents > 0);
const paginatedEvents = computed(() => {
    const start = (currentPage.value - 1) * pageSize;
    return history.value.events.slice(start, start + pageSize);
});

watch([history, selectedAssetId, selectedEventType, fromDate, toDate], () => {
    const maxPage = Math.max(1, Math.ceil(history.value.events.length / pageSize));
  ...
  <section class="history-page" aria-labelledby="history-title">
    ...
    <div class="page-heading">
      <div>
        ...
        <h1 id="history-title">{{ t('reports.history.title') }}</h1>
        <p>{{ t('reports.history.subtitle') }}</p>
      </div>
    </div>

    <section v-if="!canViewHistory && !loading" class="access-banner" aria-live="polite">
      <span class="material-icons" aria-hidden="true">lock</span>
      <div>
        <h2>{{ t('reports.history.access-title') }}</h2>
        <p>{{ t('reports.history.access-description') }}</p>
      </div>
    </section>
    ...
    <template v-else>
      <section class="filter-card four-columns" aria-label="Operational history filters">
        ...
        <label class="filter-field">
          <span>{{ t('reports.history.filter-to') }}</span>
          <input
            type="date"
        ...
        <label class="filter-field">
          <span>{{ t('reports.history.filter-asset') }}</span>
          <select :value="selectedAssetId" @change="selectAsset($event.target.value)">
            <option :value="0">{{ t('reports.history.all-assets') }}</option>
        ...
        <label class="filter-field">
          <span>{{ t('reports.history.filter-type') }}</span>
          <select :value="selectedEventType" @change="selectEventType($event.target.value)">
            <option value="all">{{ t('reports.history.types.all') }}</option>
            <option value="reading">{{ t('reports.history.types.reading') }}</option>
            <option value="alert">{{ t('reports.history.types.alert') }}</option>
            <option value="incident">{{ t('reports.history.types.incident') }}</option>
          </select>
        </label>
      </section>

      <section class="summary-grid" aria-label="Operational history summary">
        <article class="summary-card accent-blue">
          <span>{{ t('reports.history.summary-events') }}</span>
          <strong>{{ history.totalEvents }}</strong>
        </article>
        <article class="summary-card accent-green">
          <span>{{ t('reports.history.summary-readings') }}</span>
          <strong>{{ history.readingsCount }}</strong>
        </article>
        <article class="summary-card accent-amber">
          <span>{{ t('reports.history.summary-alerts') }}</span>
          <strong>{{ history.alertsCount }}</strong>
        </article>
        <article class="summary-card accent-red">
          <span>{{ t('reports.history.summary-incidents') }}</span>
          <strong>{{ history.incidentsCount }}</strong>
        </article>
      </section>

      <section class="table-card" aria-labelledby="history-table-title">
        <div class="section-heading">
          <div>
            <h2 id="history-table-title">{{ t('reports.history.table-title') }}</h2>
            <p>{{ t('reports.history.table-subtitle', {from: effectiveFromDate, to: effectiveToDate}) }}</p>
          </div>
        </div>
        ...
        <template v-if="hasEvents">
          <div class="history-table-wrapper">
            <table class="history-table">
              <thead>
                <tr>
                  <th>{{ t('reports.history.table.date') }}</th>
                  <th>{{ t('reports.history.table.asset') }}</th>
                  <th>{{ t('reports.history.table.type') }}</th>
                  <th>{{ t('reports.history.table.detail') }}</th>
                  <th>{{ t('reports.history.table.value') }}</th>
                  <th>{{ t('reports.history.table.severity') }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="event in paginatedEvents" :key="event.id">
                  ...
                  <td>
                    ...
                    <span>{{ event.assetLocation }}</span>
                  </td>
                  <td>
                    <span class="event-type">
                      <span class="material-icons" aria-hidden="true">{{ event.icon }}</span>
                      {{ t(eventTypeLabelKey(event.eventType)) }}
                    </span>
                  </td>
                  <td>{{ t(event.messageKey, event.messageParams) }}</td>
                  <td>{{ event.value }}</td>
                  <td>
                    <span class="severity-pill" :class="severityClass(event.severity)">
          ...
          <list-pagination v-model="currentPage" :total="history.events.length" :page-size="pageSize" />
        </template>

        <div v-else class="empty-state">
          <span class="material-icons" aria-hidden="true">manage_search</span>
          <h2>{{ t('reports.history.empty-title') }}</h2>
          <p>{{ t('reports.history.empty-description') }}</p>
        </div>
      </section>
    </template>
  </section>
